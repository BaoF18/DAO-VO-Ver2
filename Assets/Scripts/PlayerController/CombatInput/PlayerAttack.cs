...
public class PlayerAttack : MonoBehaviour
{
    ...
    [Tooltip("Khóa/Mở chức năng đánh (Ví dụ: tắt khi đang nói chuyện NPC)")]
    public bool isCombatUnlocked = false;

    [Header("Combo settings")]
    [Tooltip("Thời gian tối đa để nhấp chuột nối combo tiếp theo")]
    public float comboWindow = 0.5f;
    [Tooltip("Độ trễ tối thiểu giữa 2 lần click để chống spam")]
    public float minClickDelay = 0.25f;
    ...
    private void Update()
    {
        ...
        {
            // =======================================================
            // CHỐT CHẶN MANA: Hỏi xem có đủ 30% Mana để ra đòn không?
            if (PlayerHealth.Instance != null && !PlayerHealth.Instance.CanAttack())
            {
                // Nếu hết sức (dưới 30%), nhân vật sẽ đứng im không chém nữa
                Debug.Log("Hụt hơi! Yêu cầu trên 30% Mana để xuất chiêu!");
                return;
            }
            // =======================================================

            // Chặn click nếu khoảng cách giữa 2 lần bấm quá ngắn (Spam)
    ...
    private void ExecuteAttack(int step)
    {
        ...
        // 3. TRỪ MANA (Hoạt ảnh bắt đầu thì thanh xanh biển mới tụt)
        if (PlayerHealth.Instance != null)
        {
            PlayerHealth.Instance.ConsumeMana();
        }
